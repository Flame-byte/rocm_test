...
__global__ void forward_kernel(const __half* __restrict__ Q,
    ...
    // 加载Q_i到共享内存时，仅加载有效行
    for (int idx = threadIdx.y * blockDim.x + threadIdx.x; idx < BLOCK_ROWS * head_dim; idx += blockDim.x * blockDim.y) {
    ...
    for (int j = 0; j < seq_len; j += BLOCK_COLS) {
        ...
        // ---- 加载K_j到共享内存 ----
        for (int idx = threadIdx.y * blockDim.x + threadIdx.x; idx < BLOCK_COLS * head_dim; idx += blockDim.x * blockDim.y) {
            const int local_row = idx / head_dim;
            const int local_col = idx % head_dim;
