...
int main() {
    // Parameters
    const int seq_len = 1024; // Example sequence length
    ...
    flash_encoding(d_Q, d_K, d_V, d_O, seq_len, seq_len, head_dim, num_heads_q, num_heads_kv);
    ...
    std::cout << "Elements with significant error (>1e-2): " << error_count 
              << " out of " << (seq_len * num_heads_q * head_dim) << std::endl;

        // // 打印所有误差大于1e-3的元素及其位置
        // std::cout << "Elements with error > 1e-3 (index, cpu, gpu, error):" << std::endl;
        // for (int i = 0; i < seq_len * num_heads_q * head_dim; i++) {
        //     float cpu_val = __half2float(h_O_cpu[i]);
        //     float gpu_val = __half2float(h_O[i]);
        //     float error = fabs(cpu_val - gpu_val);
        //     if (error > 1e-3) {
        //         int seq = i / (num_heads_q * head_dim);
        //         int head = (i / head_dim) % num_heads_q;
        //         int dim = i % head_dim;
        //         std::cout << "  [seq=" << seq << ", head=" << head << ", dim=" << dim << "] "
        //                   << "CPU=" << cpu_val << ", GPU=" << gpu_val << ", error=" << error << std::endl;
        //     }
        // }
