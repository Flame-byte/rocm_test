...
pub fn byte_pair_encode(piece: &[u8], ranks: &HashMap<Vec<u8>, Rank>) -> Vec<Rank> {
    if piece.len() == 1 {
        return match ranks.get(piece) {
            Some(rank) => vec![*rank],
            None => {
                // If single byte isn't in the vocabulary, return a default value
                // or handle it in some other way
                println!("Warning: Single byte token not found in vocabulary: {:?}", piece);
                vec![]
            }
        };
    }
    _byte_pair_merge(ranks, piece)
        ...
        .map(|part| {
            ...
            match ranks.get(slice) {
                Some(rank) => *rank,
                None => {
                    // Handle missing token case
                    println!("Warning: Token not found in vocabulary: {:?}", slice);
                    // Return something sensible like the byte values or a sentinel
                    if slice.len() == 1 {
                        0 // Sentinel value
                    } else {
                        // Recursively tokenize this unknown sequence
                        let mut sub_tokens = Vec::new();
                        for b in slice {
                            // Create a single-byte Vec<u8> for lookup
                            let byte_vec = vec![*b];
                            if let Some(r) = ranks.get(&byte_vec) {
                                sub_tokens.push(*r);
                            } else {
                                println!("Warning: Byte {} not found in vocabulary", b);
                            }
                        }
                        // Use first token or sentinel if empty
                        sub_tokens.first().copied().unwrap_or(0)
                    }
                }
            }
        })
...
pub fn byte_pair_split<'a>(piece: &'a [u8], ranks: &HashMap<Vec<u8>, Rank>) -> Vec<&'a [u8]> {
    if piece.len() <= 1 {
        return vec![piece]; // Just return the piece if it's 1 byte or less
    }
    
    let parts = _byte_pair_merge(ranks, piece);
    if parts.len() < 2 {
        // Something went wrong, just return the original piece
        return vec![piece];
    }
