...
__global__ void forward_kernel_tiled(const __half* __restrict__ Q,
    ...
    const int j_end = min(seq_len, j_start + KV_TILE_SIZE);
    ...
    for (int j = j_start; j < j_end; j += BLOCK_COLS) {
        ...
        if (q_row < BLOCK_ROWS && k_col < BLOCK_COLS) {
            ...
            if (global_q_idx >= global_k_idx && global_q_idx < seq_len && global_k_idx < seq_len) {
...
void flash_attention_tiled(const __half* Q,
                           const __half* K,
                           const __half* V,
                           __half* O,
                           const int seq_len,
    ...
    dim3 grid(num_heads_q, (seq_len + BLOCK_ROWS - 1) / BLOCK_ROWS, (seq_len + KV_TILE_SIZE - 1) / KV_TILE_SIZE);
    dim3 block(BLOCK_ROWS, BLOCK_COLS);

    forward_kernel_tiled<<<grid, block, shared_mem_size>>>(Q, K, V, O, seq_len, head_dim, num_heads_q, num_heads_kv);
}
