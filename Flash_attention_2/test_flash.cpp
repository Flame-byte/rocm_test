...
int main() {
    ...
    std::cout << "Output O_cpu (first head, first row):" << std::endl;
    for (int i = 0; i < head_dim; ++i) {
        std::cout << __half2float(h_O_cpu[i]) << " ";
    }
    std::cout << std::endl;
    ...
    // Print the GPU output first head, first row
    std::cout << "GPU output (first head, first row):" << std::endl;
    for (int i = 0; i < head_dim; ++i) {
        std::cout << __half2float(h_O[i]) << " ";
    }
    std::cout << std::endl;
    ...
    // Print sample comparison for first few elements
    std::cout << "\nSample comparison (first head, first few elements):" << std::endl;
    std::cout << "Index\tCPU\t\tGPU\t\tDiff" << std::endl;
    for (int i = 0; i < std::min(10, head_dim); i++) {
        float cpu_val = __half2float(h_O_cpu[i]);
        float gpu_val = __half2float(h_O[i]);
        std::cout << i << "\t" << cpu_val << "\t\t" << gpu_val << "\t\t" 
                  << fabs(cpu_val - gpu_val) << std::endl;
    }
