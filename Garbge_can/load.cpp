...
std::vector<std::pair<std::vector<uint8_t>, Rank>> load_tiktoken_bpe(const std::string &file_path) {
    ...
    std::vector<std::pair<std::vector<uint8_t>, Rank>> bpe_ranks;
    std::string line;
    while (std::getline(file, line)) {
        ...
        std::vector<uint8_t> token_uint8(token.begin(), token.end());
        bpe_ranks.emplace_back(token_uint8, static_cast<Rank>(rank));
    }
